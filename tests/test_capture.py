...
TESTSRC = ["-re", "-f", "lavfi", "-i", "testsrc=size=320x240:rate=30"]  # -re: real-time pace
...
def test_recorder_keeps_ring_and_saves_clip(tmp_path):
    rec = Recorder(tmp_path, input_args=TESTSRC, segment_seconds=1)
    rec.start()
    try:
        time.sleep(3.5)
...
def test_save_without_segments_fails(tmp_path):
    with pytest.raises(RuntimeError):
        Recorder(tmp_path, input_args=TESTSRC).save_replay()
